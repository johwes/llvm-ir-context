...
_SINK_INFO: dict[str, tuple[str, str]] = {
    ...
    "system":      ("passes string to shell — command injection if user-controlled",
                    "shell metacharacters: ; | & ` $() > < newline null-byte"),
    "popen":       ("passes string to shell — command injection if user-controlled",
                    "shell metacharacters: ; | & ` $() > < newline"),
    "execv":       ("executes program — argument injection if path or argv user-controlled",
                    "path traversal in argv[0]; null bytes in arguments; empty argv"),
    "execvp":      ("executes program via PATH search — injection if name user-controlled",
                    "PATH hijack via relative name; shell metacharacters in name"),
    "execve":      ("executes program with environment — injection if any arg user-controlled",
                    "path traversal; environment variable injection; null bytes"),
    "execle":      ("executes program with explicit env",
                    "path traversal; environment injection"),
    "execl":       ("executes program with vararg argument list",
                    "argument injection; null bytes in arguments"),
    "execlp":      ("executes program via PATH search with vararg list",
                    "PATH hijack; shell metacharacters"),
    "posix_spawn": ("spawns process — injection if path or argv user-controlled",
                    "path traversal in file_actions; argument injection"),
    # Path traversal sinks
    "open":        ("opens file — path traversal if path user-controlled",
                    "../ sequences; null bytes; symlink targets; absolute paths"),
    "openat":      ("opens file relative to dir fd — path traversal if path user-controlled",
                    "../ sequences escaping dirfd; symlink attacks"),
    "creat":       ("creates/truncates file — path traversal if path user-controlled",
                    "../ sequences; symlink targets; sensitive file paths"),
    "fopen":       ("opens file stream — path traversal if path user-controlled",
                    "../ sequences; null bytes in path; symlink targets"),
    "fopen64":     ("opens large file stream — path traversal if path user-controlled",
                    "../ sequences; null bytes in path"),
    "freopen":     ("reopens stream to new path — path traversal if path user-controlled",
                    "../ sequences; symlink targets"),
    "unlink":      ("deletes file — path traversal if path user-controlled",
                    "../ sequences; symlink attacks; sensitive file deletion"),
    "unlinkat":    ("deletes file relative to dir fd — path traversal if path user-controlled",
                    "../ sequences escaping dirfd"),
    "remove":      ("removes file or directory — path traversal if path user-controlled",
                    "../ sequences; symlink attacks"),
    "rename":      ("renames file — path traversal if either path user-controlled",
                    "../ sequences in src or dst; atomic overwrite of sensitive files"),
    "renameat":    ("renames file relative to dir fds — path traversal if path user-controlled",
                    "../ sequences escaping dirfd"),
    "rmdir":       ("removes directory — path traversal if path user-controlled",
                    "../ sequences; symlink attacks"),
    "mkdir":       ("creates directory — path traversal if path user-controlled",
                    "../ sequences; sensitive directory creation"),
    "mkdirat":     ("creates directory relative to dir fd",
                    "../ sequences escaping dirfd"),
    "chmod":       ("changes file permissions — path traversal if path user-controlled",
                    "../ sequences; symlink attacks on permission-sensitive files"),
    "chown":       ("changes file ownership — path traversal if path user-controlled",
                    "../ sequences; privilege escalation via ownership change"),
    "lchown":      ("changes symlink ownership — path traversal if path user-controlled",
                    "../ sequences"),
    "symlink":     ("creates symlink — path traversal if either path user-controlled",
                    "symlink to sensitive target; ../  in link name"),
    "symlinkat":   ("creates symlink relative to dir fd",
                    "../ sequences escaping dirfd"),
    "link":        ("creates hard link — path traversal if either path user-controlled",
                    "../ sequences; hard link to sensitive file"),
    "linkat":      ("creates hard link relative to dir fds",
                    "../ sequences escaping dirfd"),
    "stat":        ("stats file — path traversal if path user-controlled",
                    "../ sequences; TOCTOU races between stat and subsequent open"),
    "lstat":       ("stats symlink — path traversal if path user-controlled",
                    "../ sequences"),
    "access":      ("checks file accessibility — TOCTOU if path user-controlled",
                    "TOCTOU race between access() and subsequent open(); ../  sequences"),
    "faccessat":   ("checks file accessibility relative to dir fd",
                    "TOCTOU race; ../  sequences escaping dirfd"),
}
